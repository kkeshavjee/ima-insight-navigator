...
const DaySchedulePane: React.FC<DaySchedulePaneProps> = ({ onSelectPatient, selectedPatientId }) => {
  ...
  return (
    <div className="h-full bg-slate-100 p-4 rounded-lg shadow-md overflow-y-auto">
      <div className="flex items-center gap-2 mb-4">
        <Clock className="w-5 h-5 text-slate-600" />
        <h2 className="text-xl font-semibold text-slate-700">Day Schedule</h2>
      </div>
      <ul className="space-y-3">
        {mockAppointments.map(appt => (
          <li
            ...
            className={`p-3 rounded-md shadow hover:bg-sky-100 cursor-pointer transition-all duration-200 ease-in-out transform hover:scale-[1.02]
                        ${selectedPatientId === appt.id ? 'bg-sky-200 ring-2 ring-sky-500 shadow-lg' : 'bg-white hover:shadow-md'}`}
          >
            <div className="flex items-center gap-2 mb-1">
              <User className="w-4 h-4 text-sky-600" />
              <div className={`font-medium ${selectedPatientId === appt.id ? 'text-sky-800' : 'text-sky-700'}`}>
                {appt.name}
              </div>
            </div>
            <div className={`text-sm ${selectedPatientId === appt.id ? 'text-slate-700' : 'text-slate-600'}`}>
